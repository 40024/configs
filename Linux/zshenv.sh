...
function load_zsh_plugins {
    ...
    # Load Plugins
    hyde_plugins=(git zsh-256color zsh-autosuggestions)
    plugins+=("${plugins[@]}" "${hyde_plugins[@]}" git zsh-256color zsh-autosuggestions)
    # Deduplicate plugins
    plugins=("${plugins[@]}")
    plugins=($(printf "%s\n" "${plugins[@]}" | sort -u))
...
if [ -t 1 ]; then
    ...
    # User defined alias
    alias m="python 'Reference Lists/Food/_Macros/Calculator/macro_calculator.py'"
    ...
    alias un='$PM remove' 
    alias up='$PM upgrade' 
    alias pl='$PM search installed' 
    alias pa='$PM search all' 
    alias vc='code' 
